...
class StripErrorsTest(TestCase):
    ...
    def test_strip_debug(self):
        input_data = {
            "debug": "test endpoint identifier",
            "action": "accept"
        }
        expected_output = {
            "action": "accept",
...
class StripErrorsDisabledTest(TestCase):
    ...
    def test_strip_debug_disabled(self):
        input_data = {
            "debug": "debug endpooint identifier",
            "action": "accept"
        }
        request = self.factory.post(
            "/dummy/",
...
class ApiTouchReject(TestCase):
    ...
    def test_touch_box_not_assigned_to_vehicle(self):
        payload={
            "card_id": "03000000" #Card is not an operator card
        }
        response = self.client.post(
            TOUCH_URL,
            data=json.dumps(payload),
    ...
    def test_touch_lock_no_booking(self):
        ...
        box.save()
        payload={
            "card_id": "04000000"#neither card nor user is operator 
        }
        response = self.client.post(
            TOUCH_URL,
            data=json.dumps(payload),
...
class ApiTouchLock(TestCase):
    ...
    def test_touch_user_with_permission_from_booking_on_box_lock(self):
        ...
        driverprofile.save()
        payload={
            "card_id": "03000000"#admin's (booking owner) non operator card
        }
        response = self.client.post(
            TOUCH_URL,
            data=json.dumps(payload),
    ...
    def test_touch_user_with_permission_from_current_booking_lock(self):
        ...
        driverprofile.save()
        payload={
            "card_id": "04000000"#user's (booking owner) non operator card
        }
        response = self.client.post(
            TOUCH_URL,
            data=json.dumps(payload),
