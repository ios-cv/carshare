...
def require_user_can_access_bookings(view_func=None):
    """
    Decorator for views that checks that the user has at least one account affiliation
    through which they are allowed to access (i.e. lock/unlock/drive) bookings.
    """
...
def require_user_can_view_bookings(view_func=None):
    """
    Decorator for views that checks that the user has at least one account affiliation
    through which they are allowed to view bookings.

    This decorator should only be used to gate whether the user is allowed to interact
    with the viewing of bookings in the system. It doesn't provide any guarantee as to
    whether the user is actually allowed to drive any given booking (or any bookings at
    all).
    """
