...
class UpdatePurchaseOrderForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            ...
            Div(
                HTML(
                    "<input type='text' name='business_purchase_order' value='{{ ba.purchase_order_update_form.business_purchase_order.value }}' class='{% if ba.successfully_updated %}bg-green-200{% endif %} rounded-l-md mt-1 text-sm text-gray-900 h-full'>"
                ),
                Submit(
                    "submit",
                    "Update",
                    css_class="inline-flex items-center rounded-e-md border border-transparent bg-teal-600 px-3 h-full mt-1 text-sm font-medium leading-4 text-white shadow-sm hover:bg-teal-700 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2",
                ),
